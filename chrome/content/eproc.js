...
Eproc = {
    ...
    init: function()
    {
        ...
        switch(this.page) {
            case 'menu_vara':
                ...
                ul.find('li').each(function(li)
                {
                    if ((p = $(this).find('p > font[face=verdana][size=2][color=green] > b')).size()) {
                        $(this).children(':not(p)').wrapAll('<li></li>').closest('li').appendTo(div.find('ul:last'));
                        p.closest('font').wrap('<li></li>').closest('li').appendTo(div).append('<ul></ul>');
                    } else {
                        $(this).children(':not(p)').wrapAll('<li></li>').closest('li').appendTo(div.find('ul:last'));
                        $(this).remove();
                    }
                ...
                ul.remove();
                div.find('li > font').css({cursor: 'pointer'}).click(
                    function() { $(this).closest('ul').find('li ul').slideUp(250).end().end().next('ul').slideToggle(250); }
                ).next('ul').filter(':not(:first)').hide();
                break;

            case 'menu_movimentacao_bloco':
            case 'menu_localizacao_bloco':
                $('select[name=cod_localizador]:eq(0)').prepend('<option value="">Todos os localizadores</option>').attr('value', '').insertBefore('form:eq(1) input[type=submit]').before('<font face="verdana" size="3"><br>&</font> <font face="verdana" size="2"><b>Localizador:</b></font> ').after('<br />');
                $('form:eq(1) select:eq(0)').prepend('<option value="">Todos os eventos</option>').attr('value', '');
                for (i = 0; i < 3; i++) $('form:eq(1) select:eq(1)').insertBefore('form:eq(1) input[type=submit]');
                $('form').next('br').remove().end().filter(':not(:eq(1)):not(:last)').remove();
                $('form:eq(0)').submit(function(e)
                {
                    $(this).find('select').each(function()
                    {
                        if (this.value == '' || (this.value == 0 && this.name == 'cod_assunto_com')) {
                            $(this).attr('disabled', 'disabled');
                        }
                    });
                    var evento = $(this).find('select[name=cod_evento_com]');
                    if (evento.get(0).value > 0) {
                        if ($(this).find('select[name=cod_assunto_com]').get(0).value > 0 && $(this).find('select[name=cod_localizador]').get(0).value > 0) {
                        } else if ($(this).find('select[name=cod_assunto_com]').get(0).value > 0) {
                        } else if ($(this).find('select[name=cod_localizador]').get(0).value > 0) {
                            evento.attr('name', 'cod_evento_localizador');
                        } else {
                            evento.attr('name', 'cod_evento');
                        }
                    }
            ...
            case 'movimentacao_bloco':
            case 'localizacao_bloco':
                $('#body table:eq(0)').next('br').eq(0).remove();
                GM_addStyle(''
                    + 'table { font-family: verdana; font-size: 10pt; }'
                    + 'table thead th { color: green; }'
                    + '#dhtmltooltip { font-family: verdana; font-size: 10pt; }'
                );
                $('font[face=verdana][size=2][color=black]').each(function()
                {
                    $(this).html($(this).html().replace('&amp;nbspAssunto', 'e&nbsp;Assunto'));
                });
                this.processos.getData($('table').get(1));
            ...
            case 'altera_localizador':
            case 'status_justica_gratuita':
            case 'status_manifestacao_mp':
            case 'status_tutela':
            case 'status_prioridade':
                if (document.referrer.split(/(\/eproc\/|\.php)/)[2] == 'movimentacao_bloco' || document.referrer.split(/(\/eproc\/|\.php)/)[2] == 'localizacao_bloco') $('input[type=button]').remove();
                break;

            case 'altera_localizador_bd':
            case 'status_justica_gratuita_bd':
            case 'status_manifestacao_mp_bd':
            case 'status_tutela_bd':
    ...
    abreJanela: function(url, target, little) {
        openWindow = Eproc.windows[target];
        if (openWindow && typeof openWindow == 'object' && openWindow.document) {
            openWindow.focus();
        } else {
            Eproc.windows[target] = window.open(url, target, 'height=350,width=' + (little ? 600 : 760) + ',status=yes,toolbar=no,menubar=no,location=no, left=' + ((screen.width - (little ? 600 : 760)) / 2) + ', top=' + ((screen.height - 350) / 2));
        }
    },
    // }}}
    // {{{ Eproc.fixMenu
    fixMenu: function()
    {
        if((menu = $('table:has(img[src $= "imagens/logo_novo2.jpg"])')).size()) {
            this.menu = menu;
            this.menu.css({fontFamily: 'Verdana', fontSize: '10pt'});
            this.menu.attr({cellPadding: 0, cellSpacing: 0, align: 'center'});
            if ((menuLink = this.menu.find('tr:eq(0) td:eq(4):has(a[href^=menu_])')).size()) {
    ...
    fixSubmenu: function()
    {
        function doIt(submenu)
        {
            submenu.find('a').appendTo(this.submenu = $('<div></div>').css({position: 'absolute', background: '#cccccc', padding: '2px', border: '1px solid #888'}).width(this.menuLink.width()).hide());
            this.submenu.appendTo(this.menuLink.find('div')).find('a').css({display: 'block'});
            submenu.closest('table').remove();
            var self = this;
            this.menuLink.hover(function(e) { self.submenu.show(); }, function(e) { self.submenu.hide(); })
        }
        if (this.menuLink && (submenu = $('table td[bgcolor=#CCCCCC][width=760]')).size()) {
            doIt(submenu);
        } else if (this.menuLink && (submenu = $('table td[bgcolor=#cccccc][width=760]')).size()) {
            doIt(submenu);
        }
    },
    // }}}
    ...
    fixTopic: function()
    {
        if (this.menu && (center = (topico = $('img[src $= "imagens/topico.gif"]')).closest('center')).size()) {
            cell = this.menu.get(0).insertRow(1).insertCell(0);
            topico.closest('font').appendTo(cell);
            center.find('font[size=2][color=#008080]:eq(0),font[size=2][color=green]:eq(0)').appendTo(cell);
            this.menu.find('tr:eq(1) td').attr({colspan: this.menu.get(0).rows[0].cells.length, align: 'center'});
            if ($.trim(center.text()) == '') {
                center.remove();
            }
        }
    ...
    {
        i = 0;
        $.each(Eproc.windows, function(w, win)
        {
            if (typeof win == 'object' && win.document) {
                i++;
            } else {
                delete Eproc.windows[w];
            }
        });
        if (i > 0) {
            if (confirm('Esta página possui ' + i + ' documentos abertos.\nDeseja fechá-los?')) {
                $.each(Eproc.windows, function(w, win) {
                    win.close();
                });
            }
...
// }}}
// {{{ Eproc.processos.buscaPrioridades
Eproc.processos.buscaPrioridades = function()
{
...
// }}}
// {{{ Eproc.processos.getData
Eproc.processos.getData = function(table)
{
    this.filters = {
        cod_evento_com: '',
        cod_assunto_com: '',
    ...
    location.search.split('?')[1].split('&').forEach(function(filter, f, filters)
    {
        var v = filter.split('=');
        this.filters[v[0]] = v[1];
    }, this);
    if (table.rows[1].cells.length > 1) {
        var self = this;
        $(table.rows).filter(':not(:first)').each(function()
        {
            self.push(new Processo(this));
        });
    }
}
// }}}
// {{{ Processo
Processo = function() { this.init.apply(this, arguments); }
Processo.prototype = {
    advogados: [],
    lembrete: '',
    al: '',
    ...
    init: function(row)
    {
            ...
            documentos: [(Eproc.page == 'localizacao_bloco' ? 9 : 10), />([^<]+)<\/a>/g, function(docs)
            {
                if (docs) {
                    for (d = 0; d < docs.length && (doc = docs[d]); d++) {
                        docs[d] = doc.match(/>([^<]+)<\/a>/)[1].match(/([A-Z]+)\d+/);
                        docs[d] = { name: docs[d][0], type: docs[d][1] }
                    }
                    return docs;
                } else {
                    return [];
        ...
        for (n in fields) {
            ...
            if ($.isArray(field) && field.length > 1) {
                ...
                x = field[1];
                if (field.length == 2) {
                    this[n] = row.cells[i].innerHTML.match(x)[1];
                } else if (field.length == 3 && typeof (t = field[2]) == 'boolean') {
                    if (t && (test = row.cells[i].innerHTML.match(x))) this[n] = test[1];
                } else if (field.length == 3 && $.isFunction(f = field[2])) {
                    this[n] = f(row.cells[i].innerHTML.match(x));
                }
            } else {
                this[n] = field;
            }
    ...
    // }}}
    // {{{ Processo.buscaPrioridade
    buscaPrioridade: function()
    {
        $(this.cell).html('<img src="chrome://global/skin/throbber/Throbber-small.gif" width="16" height="16" alt="Carregando..." />');
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.open('GET', 'status_prioridade.php?num_processo=' + this.numproc, true);
        var self = this;
        xmlhttp.onreadystatechange = (function() { return function() { return self.onPrioridade.call(self, xmlhttp); }; })();
        xmlhttp.send(null);
    },
    // }}}
    // {{{ Processo.onPrioridade
    onPrioridade: function(xmlhttp)
    {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            this.pri = xmlhttp.responseText.match(/document\.forms\[0\]\.prioridade_atendimento\.value = "([^"]?)"/)[1] || '-';
            Eproc.table.onPrioridade(this.cell, this);
        }
...
// }}}
// {{{ Table
Table = function() { this.init.apply(this, arguments); }
Table.prototype = {
    // {{{ Table.init
    pri: false,
    init: function(processos)
    {
        this.table = this.getTable();
        $(this.table).append('<tbody></tbody>');
        for (p = 0; p < processos.length && (processo = processos[p]); p++) {
            (row = $(this.table.insertRow(this.table.rows.length)).get(0)).setUserData('index', p, null);
            $(row.insertCell(0)).html('<input type="checkbox" name="processo_' + processo.numproc + '" value="' + processo.numproc + '" />');
            $(row.insertCell(1)).html('<a href="consulta_processo.php?num_processo_consulta=' + processo.numproc + '" target="_blank">' + processo.numproc + '</a>');
            $(row.insertCell(2)).html(processo.advogados[0] ? processo.advogados.join('<br />') : 'Sem ADV.');
            ...
            if (processo.pri) {
                $(row.insertCell(10)).html('<a style="text-decoration: none; color: ' + (processo.pri == 'S' ? 'red; font-weight: bold' : 'black') + ';" href="status_prioridade.php?num_processo=' + processo.numproc + '">' + processo.pri + '</a>');
            } else {
                var cell = $(row.insertCell(10)).html('<span  onMouseover=\'ddrivetip("Clique em \\"carregar\\" no título desta coluna", 250);\' onMouseout="hideddrivetip();" >?</span>');
                processo.cell = cell;
            }
            ...
            if (processo.documentos.length > 0) {
                ...
                for (d = 0; d < processo.documentos.length && (doc = processo.documentos[d]) && (link = doc.link); d++) {
                    div.append($('<a></a>').attr({
                        href: 'download_documento.php?num_protocolo=' + link[0] + '&seq_documento=' + link[1] + '&cod_seg=' + link[2],
        ...
        for (c in cols) {
            ...
            if (c != 'checkbox') {
                th.prepend($('<div></div>').css({position: 'absolute', border: '1px solid #888', background: $(document.body).attr('bgcolor')}).hide()).hover(
                    function(e){(div = $(this).find('div')).show().css({top: $(this).position().top - $(div).height() + 'px', left: $(this).position().left + ($(this).width() / 2) - ($(div).width() / 2) + 'px'});},
                    function(e){$(this).find('div').hide();}
                );
                if (c != 'numproc') {
                    var self = this;
                    (div = th.find('div')).append(ocultar = $('<span>ocultar</span>').css({cursor: 'pointer'}).click(function(e)
                    {
                ...
                ordenar.append($('<small>[a-z]</small>').css({cursor: 'pointer'}).click(function(e)
                {
                    var col = $(e.target).closest('th').get(0).getUserData('name');
                    self.sort(col, 1);
                }));
                ordenar.append(' ').append($('<small>[z-a]</small>').css({cursor: 'pointer'}).click(function(e)
                {
                    var col = $(e.target).closest('th').get(0).getUserData('name');
                    self.sort(col, -1);
                }));
                if (c == 'pri' && !this.pri) {
                    div.append($('<span>carregar</span>').css({cursor: 'pointer'}).click(function(e)
                    {
        ...
        $(this.table).find('td').attr({vAlign: 'top'});
    },
    // }}}
    // {{{ Table.filterCol
    filterCol: function(col, value)
    {
        if (confirm('Deseja retirar desta listagem os processos com ' + col + ' "' + value + '"?')) {
            for (tbody = $('tbody', this.table).get(0), r = tbody.rows.length - 1; r >= 0 && (row = tbody.rows[r]); r--) {
                if (Eproc.processos[r][col] == value) {
                    Eproc.processos.splice(r, 1);
    ...
    getTable: function()
    {
        return $('<table></table>').attr({border: 1, cellPadding: 1, cellspacing: 0}).get(0);
    },
    // }}}
    // {{{ Table.hideCol
    hideCol: function(c)
    ...
    onClick: function(e)
    {
        if ($(e.target).closest('tbody').size()) {
            if ((link = $(e.target).closest('a')).size()) {
                ...
                var name = link.closest('table').find('thead tr:eq(0) th:eq(' + td.cellIndex + ')').get(0).getUserData('name');
                switch (name) {
                    case 'numproc':
                    /*
                        var div = $('<div></div>').css({
                        ...
                        }).appendTo(td).html('<img src="chrome://global/skin/throbber/Throbber-small.gif" width="16" height="16" alt="Carregando..." />');
                    ...
                    */
                    ...
                    case 'lembrete':
                        Eproc.abreJanela(link.get(0).href, Eproc.processos[index].numproc + name, true);
                        break;
                    ...
                    case 'tutela':
                    case 'pri':
                        Eproc.abreJanela(link.get(0).href, Eproc.processos[index].numproc + name);
                        break;
                }
            }
        }
    ...
    {
        e.preventDefault();
        openWindow = Eproc.windows[this.target];
        if (openWindow && typeof openWindow == 'object' && openWindow.document) {
            openWindow.focus();
        } else {
            Eproc.windows[this.target] = window.open(this.href, this.target, 'menubar=0,resizable=1,status=0,toolbar=0,location=0,menubar=0,directories=0,scrollbars=1');
        }
    },
    // }}}
    // {{{ Table.onPrioridade
    ...
    {
        this.pri = true;
        $(cell).html(link = $('<a href="status_prioridade.php?num_processo=' + processo.numproc + '"></a>').css({textDecoration: 'none'}).html(processo.pri));
        if (processo.pri == 'S') {
            link.css({color: 'red', fontWeight: 'bold'});
        } else {
    ...
    {
        Eproc.processos.sort(function(a, b)
        {
